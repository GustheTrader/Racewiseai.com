    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button 
          ...
          onClick={onRefresh}
          disabled={isLoading}
          className="ml-2 border-betting-tertiaryPurple hover:bg-betting-tertiaryPurple/20"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-md border-4 border-betting-tertiaryPurple overflow-hidden bg-betting-darkPurple">
          <Table>
            ...
            <TableBody>
              {currentResults.length === 0 ? (
                ...
                currentResults.map((result) => (
                  ...
                  >
                    <TableCell className="font-medium text-white">{result.track_name}</TableCell>
                    <TableCell className="text-white">{result.race_number}</TableCell>
                    <TableCell className="text-white">{formatDate(result.race_date)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
        ...
        <div>
          {selectedResult ? (
            <Card className="bg-betting-darkPurple border-4 border-betting-tertiaryPurple h-full overflow-hidden">
              <CardContent className="p-4">
                ...
                <div className="bg-betting-dark p-4 rounded-md overflow-auto max-h-[500px] border border-betting-tertiaryPurple">
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {selectedResult.results_data.finishOrder.map((horse: any, index: number) => (
                              <TableRow key={index}>
                                <TableCell className="text-white">{horse.position || (index + 1)}</TableCell>
                                <TableCell className="text-white">{horse.name}</TableCell>
                                <TableCell className="text-white">{horse.jockey || 'N/A'}</TableCell>
                                <TableCell className="text-white">{horse.time || 'N/A'}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                        ...
                        <div className="mt-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {Object.entries(selectedResult.results_data.payouts).map(([bet, payout]: [string, any]) => (
                              <div key={bet} className="bg-betting-tertiaryPurple/30 p-3 rounded-md border border-betting-tertiaryPurple">
            ...
            <div className="flex items-center justify-center h-full border-4 border-dashed border-betting-tertiaryPurple rounded-md p-8 bg-betting-darkPurple">
